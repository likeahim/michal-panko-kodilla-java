...
import com.kodilla.good.patterns.challenges.order.contact.SmsInformationService;

import java.time.LocalDate;
import java.util.List;

public class TelefonBuyingService implements BuyingService {
    ...
    public TelefonBuyingService(Client client, Order order) {
        this.client = client;
        this.order = order;
    }
    ...
    public boolean finishOrder(Client client) {
        if (makeAnOrder(client)) {
            if (!order.isPaid()) {
                SmsInformationService.sendMessage(client, order, Message.ASK_FOR_PAY);
                order.setSuccess(false);
                return false;
            } else {
                order.setSuccess(true);
                SmsInformationService.sendMessage(client, order, Message.ORDER_SUCCESS);
                isDone = true;
                return true;
            }
        }
